...
function main()
	...
	while true do
		wait(0)
		local cmdResult = sampIsChatCommandDefined("drunklvl")
		if not cmdResult then
			local result = sampRegisterChatCommand("drunklvl", toggleScript)
		end
        -- if enable then
        --     printString(printint,200)
        -- end
	end
end
...
function toggleScript()
    enable = not enable
    sampAddChatMessage('ijungtas/isjungtas', -1)
end
...
function sampev.onSetPlayerDrunk(lvl)
    if enable then
    sampAddChatMessage('SRW SET Drunk Level: '..lvl, -1)
    end
end

function sampev.onSendStatsUpdate(money, lvl)
    if enable then
        printString('Drunk Level: ~r~'..lvl,1000)
    end
end
