...
function main()
    ...
    if iniData.settings.firstTime then
        ...
        sampAddChatMessage(ltu('[Sounds] Sutvarkyta leidžiamų vardų sistema'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] Trys nauji garsai: {b88d0f}!borta{1cd031}, {b88d0f}!nebebus{1cd031}, {b88d0f}!vazonas.'), 0x1cd031)
        sampAddChatMessage(ltu('-------------------------------------------------------------------'), 0x1cd031)
    end
    ...
    for i = 1, #sounds do
        for j = 1, #json_data do
            ...
            for k = 1, #chat do
                ...
                if
                    string.find(text, chat[k] .. json_data[j].nick .. ':') and string.find(text, sounds[i]) and s == 1
                 then
                    local failas = string.match(text, sounds[i])
                    failas = failas .. '.mp3'
                    failas = string.gsub(failas, '!', '')
