...
function main()
    ...
    if iniData.settings.firstTime then
        ...
        sampAddChatMessage(
            ltu('[Sounds] Dabar galima užtildyti garsus iš kitų žaidėjų. /garsai -> Leidžiami vardai'),
            0x1cd031
        )
        sampAddChatMessage(ltu('[Sounds] Pakeistas serveris į emlsound.xyz.'), 0x1cd031)
        sampAddChatMessage(ltu('-------------------------------------------------------------------'), 0x1cd031)
    end
