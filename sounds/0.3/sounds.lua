...
function main()
    ...
    if parse ~= 4 then   -- JEI ATSIRAS NAUJU ATNAUJINTI CIA
        ...
        if a then sampAddChatMessage(ltu('[Sounds] Sėkmingai ištrynėmė settings.ini. Perkrauname script.'), 0x1cd031) end
    ...
    if iniData.settings.firstTime then 
        ...
        sampAddChatMessage(ltu('[Sounds] Naujovės:'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] Bendram chat veiks garsai {ff0000}visiems.'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] Du nauji garsai: {b88d0f}!prisijunge {1cd031}ir {b88d0f}!ginsit.'), 0x1cd031)
        sampAddChatMessage(
            ltu(
                '[Sounds] Pasitikrinkite {b88d0f}moonloader/config/sounds/settings.ini{1cd031}. Jei nėra neverPlay = true'
            ),
            0x1cd031
        )
        sampAddChatMessage(ltu('[Sounds] Tada ištrinkite {ff0000}settings.ini {1cd031}failiuką!'), 0x1cd031)
        sampAddChatMessage(ltu('-------------------------------------------------------------------'), 0x1cd031)
    end
...
function download_handler(id, status, p1, p2)
    ...
    if status == dlstatus.STATUS_ENDDOWNLOADDATA then
        sampAddChatMessage(ltu('-------------------------------------------------------------------'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] {16f534}Sėkmingai {1cd031}atnaujinta. Perkraunamas script.'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] Naujovės:'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] Bendram chat veiks garsai {ff0000}visiems.'), 0x1cd031)
        sampAddChatMessage(ltu('[Sounds] Du nauji garsai: {b88d0f}!prisijunge {1cd031}ir {b88d0f}!ginsit.'), 0x1cd031)
        sampAddChatMessage(
