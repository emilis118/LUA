...
for i = 1, #fileList do
    if not doesFileExist(getGameDirectory() .. '//moonloader//lib//' .. downloadManager(fileList[i], 'fileDir')) then
        sampAddChatMessage(ltu('[Sounds] Atsiųstas reikalingi library: ' .. fileList[i]), -1)
        local scr = thisScript()
        scr:reload()
    end
end
...
local naujoves = {
    ...
    '[Sounds] Nebebus trumpam sustabdomas žaidimas perkrovus script.',
    '[Sounds] Nepaleis garsų kol nuleistas žaidimas ar įjungtas ESC.',
    '[Sounds] Apsauga nuo garsų floodinimo (3 sekundės).',
    '[Sounds] Nauji garsai.',
    '-------------------------------------------------------------------'
}
...
function allSoundList()
    local dialogText = ''
    local montagoNumber = {2, 5, 7, 9, 14, 15, 21, 27, 28, 32, 34, 36, 38, 44, 70}
    local montagoText = '{00ff00}Paulius_Montago: (' .. #montagoNumber .. '){ffffff}\n'
    local dvdNumber = {11, 23, 31, 39, 80}
    local dvdText = '{00ff00}Deividas_Deividito: (' .. #dvdNumber .. '){ffffff}\n'
    local kalantaText = ''
    local accText = ''
    local kitiText = ''
    local visiText = ''
    local spacer = 0
    for j = 1, #montagoNumber do
        for i = 1, #sounds do
            --montago, dvd, kalanta, acc, randomai, all:
            if i == montagoNumber[j] then
                spacer = spacer + 1

                if spacer == 5 then
                    montagoText = montagoText .. sounds[i] .. ' ' .. '\n'
                    spacer = 0
                else
                    montagoText = montagoText .. sounds[i] .. ' '
                end
    ...
    spacer = 0
    for j = 1, #dvdNumber do
        for i = 1, #sounds do
            --montago, dvd, kalanta, acc, randomai, all:
            if i == dvdNumber[j] then
                spacer = spacer + 1

                if spacer == 5 then
                    dvdText = dvdText .. sounds[i] .. ' ' .. '\n'
                    spacer = 0
                else
                    dvdText = dvdText .. sounds[i] .. ' '
                end
        ...
        if enable or limpMode then
            ...
            if result and button == 1 then
                ...
                if list == 1 then
                    ...
                    --         dialogText = dialogText .. sounds[i] .. '\n'
                    --     else
                    --         dialogText = dialogText .. sounds[i]
                    --     end
                    -- end
                    local garsuText = allSoundList()
                    sampShowDialog(1, 'Garsai', garsuText, ltu('Uždaryti'), nil, 2)
                end
                ...
                if list == 7 or list == 8 then
                    sampAddChatMessage(ltu('[Sounds] Atnaujinama versija į: ' .. serverVersion), 0x1cd031)
                    enableUpdate = true
                end
            end
            result = false
